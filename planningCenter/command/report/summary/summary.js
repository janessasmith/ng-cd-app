...
angular.module('reportsummaryModule', [])
    .controller('reportsummaryCtrl', function($scope, $state, dateFilter, $location, trsspliceString, plancenterService, planCtrModalService, localStorageService, trsSelectItemByTreeService, trsconfirm, trsHttpService,SweetAlert) {
        var vm = $scope.vm = {};
        ...
        var altersuccessMsg = function(msg) {
        ...
        $scope.settingTopic = function(topic) {
            var createModal = trsSelectItemByTreeService.setTopic(topic).result.then(function(result) {
                ...
                var starttime=  result.oldStartTime==result.startTime? null:dateFilter(result.startTime, format);
                plancenterService.invoke('editTimingPlanTopic', { UserIds: UserIds.join(','), TOPICID: topic.TOPICID, TimingTopicId: topic.TIMINGPLANTOPICID, Content: content, StartTime:starttime, EndTime: dateFilter(result.endTime, format) ,status:topic.STATUS}).then(function(data) {
                    getTopicList();
                });
            });
        };
