...
angular.module('PlanHotHeadLineModule', [
    ...
    .controller('PlanHotHeadLineCtrl', ['$scope', '$q', '$timeout', '$filter', 'trsHttpService', 'initSingleSelecet', 'trsEchartsService', 'hotHeadLineService', '$modal', 'initCueMonitorMoreService', 'randomTxtList', 'barEcharts', 'initHotHead', 'randomClass', '$window',
        function($scope, $q, $timeout, $filter, trsHttpService, initSingleSelecet, trsEchartsService, hotHeadLineService, $modal, initCueMonitorMoreService, randomTxtList, barEcharts, initHotHead, randomClass, $window) {
            initstatus();
            initData();
            ...
            function topNewsPaper() {
                ...
                trsHttpService.httpServer(trsHttpService.getBigDataRootUrl(), params, "get").then(function(data) {
                    barEcharts.barEcharts(data, function(data) {
                        ...
                        $scope.data.newspaperMedias.ids = $scope.status.initTopNewspaperData.ids[0];
                        newspaperList(false);
                    });
                });
            }
            ...
            $scope.$on('planNewsDetailList', function(params) {
                ...
                $scope.data.newspaperMedias.ids = params.targetScope.newspaperlistDetail.ids;
                newspaperList(false);
            });
            /**
            ...
            $scope.showGroupPaper = function() {
                ...
                trsHttpService.httpServer(trsHttpService.getBigDataRootUrl(), groupPaper, "get").then(function(data) {
                    ...
                    $scope.data.newspaperMedias.newsMediasDetailList = data.CONTENT.RESULT.CONTENT;
            ...
            function newspaperList(isPage) {
                var detailList = {
                    serviceid: "papermedia",
                    modelid: "list",
                    clusterid: $scope.data.newspaperMedias.ids,
                    page_no: $scope.data.newspaperMedias.page.page_no,
                    page_size: 100,
                };
            ...
            /**
             * [ calculateLabelWidth description] 计算标签宽度
             *  @param {[date]} [list] [description]标签数组
             *  @return {[obj]}  obj       [description] 获取宽度样式
             */
            $scope.calculateLabelWidth = function(list) {
            ...
            /**
             * [NUMSTRVALUE description]  热点头条、网名热搜 不同分辨率下 截取不同长度
             * @param {[type]} STRVALUE [description]
             */
            $scope.NUMSTRVALUE = function(STRVALUE) {
                if (window.screen.width <= '1366') {
                    STRVALUE = $filter('trsLimitTo')(STRVALUE, '15');
                } else if (window.screen.width <= '1680' && window.screen.width > '1366') {
                    STRVALUE = $filter('trsLimitTo')(STRVALUE, '18');
                } else if (window.screen.width > '1680') {
                    STRVALUE = $filter('trsLimitTo')(STRVALUE, '20');
                }
                return STRVALUE;
            };
        }
