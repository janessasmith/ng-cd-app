    ...
    .controller('addCustomMonitorController', ['$scope', '$validation', '$state', '$stateParams', 'trsHttpService', 'trsconfirm', '$q',
        function($scope, $validation, $state, $stateParams, trsHttpService, trsconfirm, $q) {
            ...
            $scope.enterEvent = function($event) {
                ...
                if ($event.target.placeholder === "请输入搜索关键字") {
                    $scope.searchChannels();
                    return;
                };
                var textInput = $(".addCustomMonitor").find("input[type='text']");
                var textArea = $(".addCustomMonitor").find("textarea");
                textInput.splice(1, 0, textArea[0]);
                for (var i = 0; i < textInput.length; i++) {
                    if ($event.target.outerHTML == textInput[i].outerHTML) {
                        i + 1 !== textInput.length?textInput[i + 1].focus():textInput[0].focus();
                        break;
                    }
                }
            };
