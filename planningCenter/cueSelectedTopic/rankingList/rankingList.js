        ...
        function($scope, $q, trsHttpService, randomTxtList, $filter, rankingList, $timeout) {
            ...
            $scope.ranklistTab = function(channel) {
                ...
                if ($scope.status.rankingTabs == '新浪') {
                    $scope.status.curchannelName = channel.CATEGORY != '推荐' ? $scope.status.sinaAcitve.title + '-' + channel.CATEGORY : $scope.status.sinaAcitve.title + '-' + "推荐";
                }
            ...
            function ranklistListDetail(channel, type) {
                ...
                if ($scope.status.rankingTabs == "新浪" && ranking.type === undefined) {
                    if (angular.isDefined($scope.status.sinaAcitve)) {
                        ranking.type = $scope.status.sinaAcitve.title;
                        ranking.channel = ranking.channel == '推荐' ? $scope.status.sinaAcitve.title + '-' + '推荐' : ranking.channel;
                    } else {
                        ranking.type = "点击量排行";
                        ranking.channel = "点击量排行-推荐";
                    }
            ...
            $scope.jumpToPage = function() {
                ...
                $scope.page.startpage = $scope.status.jumpToPageNum;
                ranklistListDetail($scope.status.sinaStatus.channel);
            };
            /*
