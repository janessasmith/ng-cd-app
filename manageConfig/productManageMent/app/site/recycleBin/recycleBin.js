...
angular.module("productManageMentAppSiteRecycleBinModule", [])
    .controller("productManageMentAppSiteRecycleBinCtrl", ["$scope", "$state", "$stateParams", "trsHttpService", 'productMangageMentAppService', 'trsconfirm', function($scope, $state, $stateParams, trsHttpService, productMangageMentAppService, trsconfirm) {
        //返回上一页
        initStatus();
        initData();
        ...
        $scope.search = function(ev) {
            if((angular.isDefined(ev) && ev.keyCode == 13) || angular.isUndefined(ev)) {
                $scope.queryReAppParams.SiteDesc = $scope.searchWord;
                initData();
            }
        };
        //还原弹窗
        ...
        function initData() {
            trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), $scope.queryReAppParams, "post")
                .then(function(data) {
