"use strict";
angular.module("appTemplateServiceCtrlModule", [

    ])
    .controller('appTemplateImportTemplateCtrl', ['$scope', "$modalInstance", "$stateParams", "initManageConSelectedService", "trsspliceString", "trsHttpService", "Upload", "trsconfirm", "host", function($scope, $modalInstance, $stateParams, initManageConSelectedService, trsspliceString, trsHttpService, Upload, trsconfirm, host) {
        init();
        initData();
        $scope.upload = function(file) {
            ...
            Upload.upload({
                ...
                // trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), resp, 'post').then(function(data) {
        ...
        $scope.confirm = function() {
            ...
            } else if (angular.isDefined($scope.filename) && $scope.status.progress != 100) {
                trsconfirm.alertType("正在上传中", "请等待上传完毕再进行操作", "error", false, function() {
                    return;
                });
            } else {
    ...
    .controller('appTemplatecreateOrEditTemplateCtrl', ['$scope', "$q", "$modal", "$validation", "$stateParams", "$modalInstance", "trsHttpService", "initManageConSelectedService", "trsspliceString", "item", "trsconfirm", "host", function($scope, $q, $modal, $validation, $stateParams, $modalInstance, trsHttpService, initManageConSelectedService, trsspliceString, item, trsconfirm, host) {
        ...
        $scope.insertFragment = function() {
            var modalInstance = $modal.open({
                templateUrl: "./manageConfig/productManageMent/app/template/template/insertFragment_tpl.html",
                scope: $scope,
                windowClass: 'man_produ_app_insert',
                backdrop: false,
                controller: "appTemplateInsertFragmentCtrl"
            });
            modalInstance.result.then(function(result) {
                if (angular.isDefined($scope.textStart)) {
                    $scope.item.TEMPTEXT = angular.isDefined($scope.item.TEMPTEXT) ? $scope.item.TEMPTEXT.substring(0, $scope.textStart) + ('<!--#include virtual="' + result.INCLUDE + '" -->\n<!--' + result.NOTE + '-->') + $scope.item.TEMPTEXT.substring($scope.textStart) : ('<!--#include virtual="' + result.INCLUDE + '" -->\n<!--' + result.NOTE + '-->');
                } else {
                    $scope.item.TEMPTEXT = angular.isDefined($scope.item.TEMPTEXT) ? $scope.item.TEMPTEXT + ('<!--#include virtual="' + result.INCLUDE + '" -->\n<!--' + result.NOTE + '-->') : ('<!--#include virtual="' + result.INCLUDE + '" -->\n<!--' + result.NOTE + '-->');
                }
            });
        };
        ...
        $scope.templateCheck = function() {
            var params = {
                ...
                TempId: $scope.item.TEMPID,
                TemplateText: $scope.item.TEMPTEXT,
                TempName: $scope.item.TEMPNAME
            };
            $scope.loadingPromise = trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "post").then(function(data) {
                var modalInstance = $modal.open({
                    templateUrl: "./manageConfig/productManageMent/app/template/template/templateCheck_tpl.html",
                    scope: $scope,
                    windowClass: 'man_produ_app_templateCheck',
                    backdrop: false,
                    resolve: {
                        data: function() {
                            return data;
                    ...
                    controller: "appTemplateTemplateCheckCtrl"
                });
    ...
    .controller('appTemplateSyncTemplateCtrl', ['$scope', "$stateParams", "data", "$modalInstance", "trsHttpService", function($scope, $stateParams, data, $modalInstance, trsHttpService) {
        init();
    ...
    .controller('appTemplateInsertFragmentCtrl', ['$scope', "$modalInstance", "trsHttpService", "$stateParams", "trsspliceString", "initManageConSelectedService", function($scope, $modalInstance, trsHttpService, $stateParams, trsspliceString, initManageConSelectedService) {
        initstatus();
        initData();
        ...
        $scope.search = function() {
            $scope.params.TEMPDESC = $scope.insertFragmentSearch;
            requestData();
        };


    }]).controller('appTemplateTemplateCheckCtrl', ['$scope', "$stateParams", "$modalInstance", "trsspliceString", "data", function($scope, $stateParams, $modalInstance, trsspliceString, data) {
        init();
