...
*/
"use strict";
angular.module("productManageMentAppTemplateModule", ['appTemplateServiceCtrlModule'])
    .controller("productManageMentAppTemplateCtrl", ['$scope', "$q", "trsHttpService", "$stateParams", '$modal', 'trsconfirm', 'trsspliceString', 'SweetAlert', '$timeout', 'initSingleSelecet', 'initVersionService', 'initManageConSelectedService', 'trsResponseHandle', 'productMangageMentAppService', function($scope, $q, trsHttpService, $stateParams, $modal, trsconfirm, trsspliceString, SweetAlert, $timeout, initSingleSelecet, initVersionService, initManageConSelectedService, trsResponseHandle, productMangageMentAppService) {
        initStatus();
        initData();
        /**
        ...
        function getTemplateRights() {
            if ($scope.status.isSite) {
                productMangageMentAppService.getRight($stateParams.site, "", "appsetsite.template").then(function(data) {
                    $scope.status.right = data;
                });
            } else {
                productMangageMentAppService.getRight("", $stateParams.parentchnl, "appsetchannel.template").then(function(data) {
                    $scope.status.right = data;
                });
            }
        }
        ...
        $scope.exportSelectedTemplate = function() {
            ...
            //window.open(trsHttpService.getWCMRootUrl() + "?serviceid=mlf_appconfig&methodname=export&HostType=" + ($scope.status.isSite ? "103" : "101") + "&HostId=" + ($scope.status.isSite ? $scope.status.params.SiteId : $scope.status.params.ChannelId) + "&ObjectIds=" + ObjectIds + "&ExportAll=false");
            var params = {
                serviceid: "mlf_appconfig",
                methodname: "export",
            ...
            //window.open(trsHttpService.getWCMRootUrl() + "?serviceid=mlf_appconfig&methodname=export&HostType=" + ($scope.status.isSite ? "103" : "101") + "&HostId=" + ($scope.status.isSite ? $scope.status.params.SiteId : $scope.status.params.ChannelId) + "&ExportAll=true");
        };
        /**
         * [exportSelectedTemplate description]导出选中模板
         * @return {[type]} [description] null
        ...
        $scope.syncTemplate = function() {
            ...
            trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "get").then(function(data) {
                trsconfirm.alertType("同步模板成功", data.TITLE, "success", false);
                $scope.data.selectedArray = [];
                // var modalInstance = $modal.open({
                //     templateUrl: "./manageConfig/productManageMent/app/template/template/syncTemplate_tpl.html",
                //     scope: $scope,
                //     windowClass: 'man_produ_app_importTemplate',
                //     backdrop: false,
                //     resolve: {
                //         data: function() {
                //             return data;
                //         }
                //     },
                //     controller: "appTemplateSyncTemplateCtrl",
                // });
        ...
        $scope.importTemplate = function() {
            var modalInstance = $modal.open({
                templateUrl: "./manageConfig/productManageMent/app/template/template/importTemplate_tpl.html",
                scope: $scope,
                windowClass: 'app_man_produ_importTemplate',
                backdrop: false,
                resolve: {
                    host: function() {
                ...
                controller: "appTemplateImportTemplateCtrl",
            });
            modalInstance.result.then(function(result) {
                initData();
            });
            ...
            trsconfirm.confirmModel("删除", "您是否确认删除！", function() {
                ...
                var params = {
                    ...
                    TempIds: trsspliceString.spliceString($scope.data.selectedArray,
                        "TEMPID", ','),
                    FolderType: $stateParams.channel ? "101" : "103",
                    FolderId: $stateParams.channel ? $stateParams.channel : $stateParams.site
                };
                trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "post")
                    .then(function(data) {
                        trsconfirm.alertType("删除模板成功", "", "success", false);
                        initData();
                    });
            });
        };
        ...
        $scope.restoreTemplate = function() {
            ...
            trsconfirm.confirmModel("还原", "您是否确认还原选中模板？", function() {
                ...
                trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "post")
                    .then(function(data) {
                        trsconfirm.alertType("还原模板成功", "", "success", false);
                        initData();
                    });
            });
        };
        ...
        function createOrEditTemplate(item) {
            var modalInstance = $modal.open({
                templateUrl: "./manageConfig/productManageMent/app/template/template/createOrEditTemplate_tpl.html",
                // scope: $scope,
                windowClass: 'man_produ_app_createNewTemplate',
                backdrop: false,
                resolve: {
                    item: function() {
                        return item;
                ...
                controller: "appTemplatecreateOrEditTemplateCtrl"
            });
