 ...
 */
'use strict';
angular.module('proManaMentAppFootModule', [

]).
controller('proMgrAppfooterRouterCtrl', ['$scope', '$stateParams', 'localStorageService', function($scope, $stateParams, localStorageService) {
    initStatus();
    initData();
    ...
    function initStatus() {
        ...
        if (angular.isDefined($stateParams.parentchnl)) {
            //判断为获取栏目访问权限
            $scope.status.rightOperType = localStorageService.get("appRightOperType_channel");
        } else {
            //判断为获取频道访问权限
            $scope.status.rightOperType = localStorageService.get("appRightOperType_site");
        }
        //从缓存中获取权限信息结束
        packageData();
        //清除缓存
