...
angular.module("productManageMentWebsiteChannelRecycleBinModule", [
    ...
    .controller("productManageMentWebsiteChannelRecycleBinCtrl", ["$scope", "$q", "productMangageMentWebsiteService", "$stateParams", "$state", "$rootScope", "trsHttpService", "trsconfirm", function($scope, $q, productMangageMentWebsiteService, $stateParams, $state, $rootScope, trsHttpService, trsconfirm) {
        initStatus();
        initData();
        //获取栏目操作权限开始
        ...
        $scope.reductionViews = function(item) {
            productMangageMentWebsiteService.recycleReductionViews({
                ...
                trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), reSiteParams, "post")
                    .then(function(data) {
                        $rootScope.$broadcast("leftAddChannel", item);
