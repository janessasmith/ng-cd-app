...
angular.module("productManageMentWebsiteColumnRecycleBinModule", [])
    .controller("productManageMentWebsiteColumnRecycleBinCtrl", ["$scope", "$q", "$stateParams", "$state", "$rootScope", "trsHttpService", "productMangageMentWebsiteService", "trsconfirm", "localStorageService", function($scope, $q, $stateParams, $state, $rootScope, trsHttpService, productMangageMentWebsiteService, trsconfirm, localStorageService) {
        initStatus();
        initData();
        //获取栏目操作权限开始
        function getChnlRights() {
            productMangageMentWebsiteService.getRight("", $stateParams.channel, "websetchannel.channel").then(function(data) {
        ...
        $scope.reductionViews = function(item) {
            productMangageMentWebsiteService.recycleReductionViews({
                ...
                trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), reSiteParams, "post")
                    .then(function(data) {
                        $rootScope.$broadcast("leftAddColum", item);
