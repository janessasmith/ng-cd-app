    ...
    angular.module("manageCfg.roleManageMent.permissionAssignment.proRange", ["manageCfg.roleManageMent.permissionAssignment.productAndModule", "manageCfg.roleManageMent.permissionAssignment.SavePerModule"])
        .controller("proRangeCtrl", ["$scope", "$timeout", "$state", "$modal", "$compile", "$element", "trsHttpService", "SweetAlert", "trsconfirm", "permissionAssignmentService", function($scope, $timeout, $state, $modal, $compile, $element, trsHttpService, SweetAlert, trsconfirm, permissionAssignmentService) {
            ...
            $scope.cTshowSelected = function(sel) {
                if (angular.isDefined($scope.authorSelectedNode) && $scope.authorSelectedNode.CLASSIFY !== sel.CLASSIFY) {
            ...
            $scope.getNodeDesc = function(node) {
                return node.MEDIANAME || node.WEBSITENAME || node.CHANNELNAME || node.ACCOUNTNAME;
            };
            //保存权限开始
            $scope.save = function() {
                var i = 0,
                    ...
                    trsconfirm.alertType("保存权限成功", "", "success", false,
                        function() {
                            ...
                            getChannelTree($scope.roleData.ROLEID, function(data) {
                                ...
                                $scope.status.expandedNodesCopy = angular.copy($scope.expandedNodes);
                                $scope.expandedNodes = [];
                                angular.forEach($scope.channelTree, function(_data, _index, _array) {
                                    angular.forEach($scope.status.expandedNodesCopy, function(__data, __index, __array) {
                                        if (_data.MEDIANAME === __data.MEDIANAME) {
                                            $scope.expandedNodes.push($scope.channelTree[_index]);
                                        }
                                    });
                                });
                                /*angular.forEach($scope.channelTree, function(data, index, array) {
                                    $scope.expandedNodes.push($scope.channelTree[index]);
                                });*/
                            });
                        });
                });
            ...
            $scope.isRootNode = function(node) {
                return node.ISONLYNODE === "true" || node.MEDIANAME === "微信";
            };
            //初始化全选按钮方法开始
            ...
            $scope.batPermissionAssignment = function(node) {
                ...
                if (node.CLASSIFY !== $scope.authorSelectedNode.CLASSIFY) {
                    trsconfirm.alertType("不允许跨产品渠道批量授权", "", "warning", false);
                    return;
                }
