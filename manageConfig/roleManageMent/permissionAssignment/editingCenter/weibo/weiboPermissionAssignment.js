'use strict';
angular.module("weiboPermissionAssignment", ["manageCfg.roleManageMent.permissionAssignment.productAndModule", "manageCfg.roleManageMent.permissionAssignment.SavePerModule"])
    .controller("weiboPermissionAssignmentCtrl", ["$scope", "$timeout", "$state", "$modal", "$compile", "$element", "$q", "localStorageService", "trsHttpService", "SweetAlert", "trsconfirm", "permissionAssignmentService",
        function($scope, $timeout, $state, $modal, $compile, $element, $q, localStorageService, trsHttpService, SweetAlert, trsconfirm, permissionAssignmentService) {
            ...
            //保存权限结束
            //获取树开始
            function getChannelTree(accountId, roleId, callback) {
                var params = {
                    "serviceid": "mlf_microblog",
                    "methodname": "queryMicroBlogRightInfo",
                    "RoleId": $scope.roleData.ROLEID,
                    "AccountId": accountId                };
            ...
            function initData() {
                ...
                $scope.$on("roleData", function(event, data) {
                    $scope.roleData = data;
                    getChannelTree($scope.accountId,$scope.roleData.ROLEID, function(data) {
            ...
            function getProductListData() {
                ...
                $scope.accountId = productListData.accountId;
