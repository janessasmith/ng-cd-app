"use strict";
/**
 * created By cc 2016-11-10 部门稿库权限设置
 */
angular.module("depPermissionAssignmentModule", []).
controller('depPermissionAssignmentCtrl', ['$scope', '$q', '$state', '$modal', '$timeout', 'trsHttpService', 'localStorageService', 'trsconfirm', 'permissionAssignmentService',
    function($scope, $q, $state, $modal, $timeout, trsHttpService, localStorageService, trsconfirm, permissionAssignmentService) {
        ...
        function getProductListData() {
            ...
            $scope.parentId = productListData.departmentId; //一级单位的ID
        }
        /**
         * [getDepartment description]获得部门
         * @return {[type]} [description]
         */
        function getDepartment() {
            var params = {
                serviceid: "nb_managerconfig",
                methodname: "queryDepartmentTreeNodeWithRight",
                RoleId: $scope.roleData.ROLEID,
                DepartmentId: $scope.parentId
            };
        ...
        $scope.cTshowSelected = function(sel) {
            permissionAssignmentService.initGroupRight(sel, $scope.roleData.ROLEID).then(function(data) {
                ...
                queryRightClassify('department', sel);
            });
        ...
        $scope.save = function() {
            ...
            $scope.loadingPromise = trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "post").then(function(data) {
                ...
                delete $scope.rightClassify;
                getDepartment($scope.parentId, $scope.roleData.ROLEID);
            });
        };
    }
]);
