/**
 * appPermissionAssignment Module
 *
 * Description app权限分配
 * Author:SMG 2016-7-21
 */
'use strict';
angular.module("appPermissionAssignmentModule", [])
    .controller("appPermissionAssignmentCtrl", ["$scope", "$timeout", "$state", "$modal", "$compile", "$element", "$q", "localStorageService", "trsHttpService", "SweetAlert", "trsconfirm", "permissionAssignmentService",
        function($scope, $timeout, $state, $modal, $compile, $element, $q, localStorageService, trsHttpService, SweetAlert, trsconfirm, permissionAssignmentService) {
            ...
            function initData() {
                ...
                $scope.$on("roleData", function(event, data) {
                    $scope.roleData = data;
                    getChannelTree($scope.siteId, $scope.roleData.ROLEID, function(data) {
