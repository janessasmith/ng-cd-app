    ...
    function($scope, $q, $stateParams, $location, $state, treeService, trsHttpService, trsconfirm, trsspliceString, userManageMentService, trsSelectItemByTreeService, trsGroupTreeLocationService, groupUserRightsService, localStorageService) {
        ...
        function initData() {
            ...
            treeService.queryGroupTreeWithSecondRight().then(function(data) {
                ...
                if (angular.isDefined($stateParams.groupid)) { //有组织id时，获取组织信息(组织名称)
                    ...
                    groupUserRightsService.initGeneralBtnRights(params, "用户管理").then(function(data) {
                        $scope.status.btnRights = data;
                    });
                }
            });
        ...
        function requestData() {
            ...
            if ($scope.data.parentGroupid != null) {
                ...
                } else if (params.Status == 1) { //用户管理：所有用户列表
                    params.methodname = "queryAllUsers";
                } else { //未分配的用户
            ...
            trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "get").then(function(data) {
                ...
                $scope.page.CURRPAGE = Math.abs($scope.page.CURRPAGE); //应对后端返回分页存在问题的情况
                $scope.users = data.DATA;
                initUserStatusSelected();
        ...
        /**
         * [getRolesToString description获取所属角色信息，用逗号分隔]
         * @return {[type]} [description]
         */
        /*function getRolesToString() {
            angular.forEach($scope.users, function(data, index) {
                var rolesArr = [];
                for (var i = 0; i < data.USERROLES.length; i++) {
                    rolesArr[i] = data.USERROLES[i].ROLENAME;
                }
                data.USERROLES = rolesArr.join(",");
            });
        }*/
        /**
            将日期字符串转换为时间戳
        **/
        ...
        $scope.dropCallback = function(event, index, item, external, type, allowedType) {
            ...
            var params = {
                ...
                Position: $scope.status.dragStartIndex > index ? 1 : 0,
            };
        ...
        /**
         * [getUserRoles description根据用户id获取用户所属角色信息]
         * @param  {[type]} items [description]
         * @return {[type]}       [description]
         */
        $scope.getUserRoles = function(item) {
            var params = {
                "serviceid": "mlf_usermanagement",
                "methodname": "queryUserRoles",
                "UserId": item.USERID
            };
            trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "get").then(function(data) {
                if (angular.isDefined(data)) {
                    ...
                    for (var i = 0; i < item.USERROLES.length; i++) {
                        rolesArr[i] = item.USERROLES[i].ROLENAME;
                    }
                    item.USERROLES = rolesArr.join(",");
                    item.subUSERROLES = item.USERROLES.substring(0, 6);
                }
            });
