...
'use strict';
angular.module("manageConfig", ["manageCfg", "manageConfigHeaderModule", "logManageMentModule"]).
controller("manageConfigCtrl", ["$scope", '$rootScope', "$state", "$location", "trsHttpService", "permissionService", "trsconfirm", function($scope, $rootScope, $state, $location, trsHttpService, permissionService, trsconfirm) {
    initStatus();
    initData();
    ...
    function getConfigureAccessData() {
        if (!angular.isDefined($scope.configureAccess)) {
            permissionService.getPermissionData()
                .then(
                    function(data) {
                        //获取模块权限，用于控制展现个模块入口
                        ...
                        if (hasRight) {
                            ...
                            // if (flag) {
                            //     routerPaths[0] === "groupmanage" ? $state.go("manageconfig.groupmanage.user", { status: 1 }) : $state.go("manageconfig." + routerPaths[0]);
                            // } 暂时隐去
                            //根据模块权限来定向路由开始结束
                        } else {
