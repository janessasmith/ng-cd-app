...
factory('trsconfirm', ['$modal', 'SweetAlert', function($modal, SweetAlert) {
        return {
            ...
            confirmModel: function(title, content, confirm, cancel) {
                return $modal.open({
            ...
            // 文件库提示
            cloudConfirmModel: function(title, content, confirm, quit) {
                return $modal.open({
                    templateUrl: "./components/service/sweetalert/view/directSign_tpl.html",
                    windowClass: 'toBeCompiled-directSign-window',
                    backdrop: false,
                    controller: "cloudDirectSignCtrl",
                    resolve: {
                        title: function() {
                            return title;
                        },
    ...
    .controller('directSignCtrl', ['$scope', "$sce", 'title', 'content', 'callback', 'cancel', function($scope, $sce, title, content, callback, cancel) {
        $scope.cancel = function() {
            if(angular.isDefined(cancel)){
                cancel();
            }
    ...
    }]).controller('cloudDirectSignCtrl', ['$scope', "$sce", 'title', 'content', 'callback', 'quit', function($scope, $sce, title, content, callback, quit) {
        $scope.cancel = function() {
            quit();
            $scope.$close();
        };
