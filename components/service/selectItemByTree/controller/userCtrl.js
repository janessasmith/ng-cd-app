...
angular.module("selectUserModule", [])
    .controller("selectUserCtrl", ["$scope", "$timeout", "$modalInstance", "trsHttpService", "trsconfirm", "globleParamsSet", "params", "selectedUser", function($scope, $timeout, $modalInstance, trsHttpService, trsconfirm, globleParamsSet, params, selectedUser) {
        init();
        ...
        function init() {
            ...
            var treeParams = {
                "serviceid": "mlf_group",
                "methodname": angular.isDefined(params) && (angular.isDefined(params) && params.rightFilter === false) ? "queryGroupTreeWithOutRight" : "queryGroupTreeWithSecondRight" /*"queryGroupTreeWithMyPath"*/
            };
        ...
        function childrenTree(node, callback) {
            if (node.HASCHILDREN == "true" && node.CHILDREN.length === 0) {
                var paramsT = {
                    "serviceid": "mlf_group",
                    "methodname": angular.isDefined(params) && (angular.isDefined(params) && params.rightFilter === false) ? "queryChildGroupsWithOutRight" : "queryChildGroupsWithSecondRight",
        ...
        $scope.getSuggestions = function(viewValue) {
            ...
            promise = $timeout(function() {
                if (viewValue !== '') {
                    ...
                    searchUsers.methodname = (angular.isUndefined(params) || params.rightFilter) ? "queryUsersByName" : "searchUserOrDept"; //过滤是否带权限
                    if ($scope.isRequest) {
                        ...
                        return trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), searchUsers, "post").then(function(data) {
                            angular.forEach(data, function(data_, index, array) {
                                data[index].viewValue = viewValue;
                            });
                            return data;
                        });
                    }
