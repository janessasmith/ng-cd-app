...
angular.module("deptAndUserModule", [])
    .controller("deptAndUserCtrl", ["$scope", "$timeout", "$modalInstance", "trsspliceString", "trsHttpService", "trsconfirm", "roleid", "selected", "title", "globleParamsSet", function($scope, $timeout, $modalInstance, trsspliceString, trsHttpService, trsconfirm, roleid, selected, title, globleParamsSet) {
        init();
        //弹窗初始化
            ...
            for (var i = 0; i < data.CHILDREN.length; i++) {
                if (data.CHILDREN[i].ISPATH === "true") {
                    if (data.CHILDREN[i].HASCHILDREN === "false" || data.CHILDREN[i].CHILDREN.length === 0) {
                        $scope.selectedNodePA = data.CHILDREN[i];
                        getGroupData($scope.selectedNodePA);
                    } else {
                        $scope.expandedPA.push(data.CHILDREN[i]);
