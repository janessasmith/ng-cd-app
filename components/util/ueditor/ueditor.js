...
'use strict';
var closeMas;
angular.module("util.ueditor", ['util.ueditorConfig', 'util.ueditorServiceModule']).directive("ueditor", ["$timeout", "$sce", "$q", "$modal", "$interval", "localStorageService", "trsUeditorConfigService", "trsconfirm", "ueditorService", "globleParamsSet", "$compile", "$window", "editPictureService", function($timeout, $sce, $q, $modal, $interval, localStorageService, trsUeditorConfigService, trsconfirm, ueditorService, globleParamsSet, $compile, $window, editPictureService) {
    return {
        restrict: 'E',
        scope: {
            list: "=",
        ...
        link: function(scope, element, attrs, ctrl) {
            ...
            ue.ready(function() {
                ...
                scope.list.HTMLCONTENT = scope.list.HTMLCONTENT.replace(/<trs_page_separator pagetitle="" firstpagetitle=""><\/trs_page_separator>/g, '<hr ispageseparator="true"/>'); //替换分页符标签，用于编辑器展示
            ...
            UE.registerUI('pagebreak', function(editor, uiName) {
                editor.registerCommand(uiName, {
                    execCommand: function() {
                        ...
                        ue.execCommand('insertHtml', '<hr ispageseparator="true"/>');
                        ue.fireEvent('contentchange');
                    }
                });
                ...
                if (scope.type === 'website') return btn;
            });
            ...
            if (angular.isDefined(scope.customButton)) {

                UE.registerUI(scope.customButton.name, function(editor, uiName) {
                    //注册按钮执行时的command命令，使用命令默认就会带有回退操作
                    editor.registerCommand(uiName, {
                        execCommand: function() {}
                    });
                    var btn = new UE.ui.Button({
                        ...
                        //提示
                        title: scope.customButton.title,
                        //需要添加的额外样式，指定icon图标，这里默认使用一个重复的icon
                        cssRules: 'background-position: -559px -39px;',
                        //点击时执行的命令
                        onclick: function() {
                            scope.customButtonFn();
                        }
                    });
                    return btn;
                });
                                ...
                                for (var i = 0; i < data.length; i++) {
                                    ...
                                    for (var j in data[i]) {
                                        ...
                                        if (j === "playUrl") {
                                            ...
                                            imgUrl = trsHttpService.getMasUrl() + "/mas/openapi/pages.do?method=exThumb&appKey=" + trsHttpService.getMasConfig() + "&id=" + data[i][j] + "&size=L";
                                        } else if (j === "typeMedia") {
            ...
            var popup = new baidu.editor.ui.Popup({
                ...
                _onImgclipping: function() {
                    ...
                    var image = {
                        ...
                        APPFILE: angular.isDefined($(img).attr("uploadpic")) ? $(img).attr("uploadpic") : $(img).attr("src").split("/")[$(img).attr("src").split("/").length - 1] //angular.isDefined($(img).attr("oldsrc")) ? $(img).attr("oldsrc") : $(img).attr("uploadpic")
                    };
                    scope.photoFileName = image.APPFILE;
                    editPictureService.editPicture(image, function(params) {
                        var myImg = new Image();
                        var sFileName = params.imgName || params.FN;
                        ...
                        myImg.onload = function() {
                            ...
                            //myWidth = myImg.width //> offsetWidth ? (offsetWidth + "px") : myImg.width;
                            var aspectRatio = myImg.width / myImg.height;
                            var width = myImg.width > 550 ? 550 : myImg.width;
                            var height = myImg.width > 550 ? 550 / aspectRatio : myImg.height;
                    ...
                    editCallback = function(params) {
                        var myImg = new Image();
                        var sFileName = params.imagName || params.FN;
                        var imageUrl = "/wcm/file/read_image.jsp?FileName=" + sFileName; //文件没有保存之后，是U0开头
                ...
                me.addListener('selectionchange', function(t, causeByUi) {
                    ...
                    if (img && img.tagName == 'IMG') {
                        ...
                        str = '<nobr>' +
                            ...
                            '<span onclick=$$._onImgclipping() class="edui-clickable pic-cut">' + "编辑" + '</span>&nbsp;&nbsp;' +
