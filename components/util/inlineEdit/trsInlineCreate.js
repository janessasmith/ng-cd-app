            ...
            $timeout(function() {
                ...
                var $sinaMiniBlogShare = function(eleShare) {
                    ...
                    $(eleShare).bind('click', function(event) {
                        ...
                        var currImgCopy = angular.copy(currImg);
                        if (currImg) {
                            var imgnameSrcArray = currImgCopy.src.split("/");
                            if (imgnameSrcArray[imgnameSrcArray.length - 1].indexOf("_600.") > 0) {
                                imgnameSrcArray[imgnameSrcArray.length - 1] = imgnameSrcArray[imgnameSrcArray.length - 1].replace(/_600./g, ".");
                                currImgCopy.src = imgnameSrcArray.join("/");
                            }
                            imgname = currImgCopy.src || currImgCopy.getAttribute('trs-src');
                            content = "";
                            imgauthor = currImgCopy.getAttribute("IMGAUTHOR") || "";
                            guid = currImgCopy.getAttribute("trs-guid");
                            channelName = currImgCopy.getAttribute("trs-channelName");
                            indexName = currImgCopy.getAttribute("trs-indexName");
                        } else if (txt) {
                            imgname = "";
                            content = txt.toString();
                            imgauthor = "";
                        }
                        if (content == "" && imgname == "") return;
                        if (angular.isDefined($stateParams.metadataid) || angular.isDefined($stateParams.metadata)) {
                            //WCM
                            if (angular.isDefined($scope.data) && angular.isDefined($scope.data.item) && $scope.data.item.MLFTYPE === '1') {
