...
angular.module('webIMModule', [
    ...
    function($scope, $q, $filter, $location, IMService, $timeout, $modal, $sce, $window, trsSelectItemByTreeService, trsHttpService, trsspliceString, IMWCMService, trsconfirm, Upload) {
        ...
        $scope.loadingPromise = LazyLoad.js($scope.dependencies, function(emoji) {
            ...
            $scope.loadingPromise = IMService.getIMInfo().then(function(data) {
                ...
                getGroupList().then(function(groups) {
                    $scope.loadingPromise = IMWCMService.getFriendList().then(function(friends) {
                        //$scope.data.friendList = dataC;
                        //好友按照CRTIME排序
                        var tempFriendList = friends.concat(groups);
                        angular.forEach(tempFriendList, function(data){
                            if(data.CRTIME) {
                                data.CRTIME = new Date(data.CRTIME).getTime();
                            }
                            else {
                                data.CRTIME = 0;
                            }
                        });
                        $scope.data.friendList = friends.concat(groups);
                    
                        //获取好友和群组的最后聊天时间
                        /*angular.forEach(dataC.concat(data), function(dataT){
                            if(dataT.FRIENDID) {
                                var params = {
                                    serviceid: "mlf_yuncloud",
                                    methodname: "getPrivateChattingRecords",
                                    receive: dataT.FRIENDID
                                };
                                $scope.loadingPromise = trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "get").then(function(data) {
                                    dataT.rank = data.DATA.length !== 0 ? data.DATA[0].HISTORCALTIME : dataT.CRTIME;
                                    dataT.rank = new Date(dataT.rank).getTime();
                                    console.log(dataT.FRIENDTRUENAME+','+dataT.rank);
                                });
                            }
                            else {
                                var params = {
                                    ...
                                    RECEIVE: dataT.groupId
                                };
                                $scope.loadingPromise = trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "get").then(function(data) {
                                    dataT.dateCreated = $filter('date')(dataT.dateCreated, 'yyyy-MM-dd HH:mm:ss');
                                    dataT.rank = data.DATA.length !== 0 ? data.DATA[0].HISTORCALTIME : dataT.dateCreated;
                                    dataT.rank = new Date(dataT.rank).getTime();
                                    console.log(dataT.name+','+dataT.rank);
                                });
                            }
                        });*/
                    });
        ...
        function doSendMessage(reciveid, msgid, content, file, msgType) {
            var defer = $q.defer();
            $scope.loadingPromise = IMService.sendMsg(reciveid, msgid, msgType, file, replaceContent(content).replace(/\n/g, '<br/>')).then(function(obj) {
        ...
        function getGroupMemberList(group) {
            ...
            RL_YTX.getGroupMemberList(GetGroupMemberListBuilder, function(data) {
                ...
                var memberIds = trsspliceString.spliceString(data, "member", ",");
                getGroupMemberHead(memberIds);
        ...
        /**
         * [getGroupMemberHead description] 群聊 获取群成员头像
         * @param  {[type]} ids [description] 传入参数
         * @return {[type]}     [description] PERPICURL传入参数显示数据头像，PERPICURL为空显示默认头像
         */
        function getGroupMemberHead(ids) {
            var params = {
            ...
            trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, 'get').then(function(data) {
                angular.forEach($scope.data.groupMembers, function(group, key) {
                    group.PERPICURL = data[key].PERPICURL;
                    if (group.PERPICURL == '') {
                        group.PERPICURL = "./components/util/IM/images/2016.jpg";
                    }
                });
            });
        }
