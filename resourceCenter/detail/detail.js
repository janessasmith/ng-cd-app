...
angular.module('resourceCenterDetailModule', [])
    .controller('resourceCenterDetailCtrl', ['$scope', '$location', '$q', '$sce', '$stateParams', '$window', '$filter', 'trsHttpService', 'resourceCenterService', 'editNewspaperService', 'trsconfirm', 'resCtrModalService', 'storageListenerService', "trsPrintService", "ueditorService", function($scope, $location, $q, $sce, $stateParams, $window, $filter, trsHttpService, resourceCenterService, editNewspaperService, trsconfirm, resCtrModalService, storageListenerService, trsPrintService, ueditorService) {
        ...
        function initStatus() {
            ...
            $scope.status = {
                ...
                "isEnterApp": false,
                "isXhsg": $stateParams.channel == "xhsg" ? true : false,
                "isJtcpg": $stateParams.channel == "jtcpg" ? true : false,
            };
        ...
        function initData() {
            ...
            $scope.loadingPromise = trsHttpService.httpServer(trsHttpService.getBigDataRootUrl(), $scope.params, "post").then(function(data) {
                if (data && data.content) {
                    ...
                    $scope.detailInfos.CONTENT = $scope.detailInfos.CONTENT.replace(/(<(?:p|div)[^>]*>(?:<(?:span|b)[^>]*>)*)(?:&nbsp;| |　)*/ig, "$1　　").replace(/(<br[^>]*>)(?:&nbsp;| |　)*/ig, "$1　　").replace(/>　　</g, "\>\<").replace(/text-indent:[^>]*em/g, "text-indent:0em");
