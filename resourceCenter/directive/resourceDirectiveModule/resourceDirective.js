"use strict";
angular.module('resourceDirectiveModule', ['ui.bootstrap.datepicker']).directive('previewTitle', ['$window','$timeout', function($window,$timeout) {
    return {
        restrict: 'EA',
        replace: true,
        transclude: true,
        ...
        template: '<div class="preview-title" ng-mouseenter="mouseenter($event)" ng-mouseleave="mousleave()"><a class="limita" data-ng-bind-html="titleText"  ng-click="openTarget()" ng-class="{\'not-read\':notRead==0}"></a>' +
            '<div class="panel panel-info extra-info" ng-show="extraInfoShow" ng-style="panelpostion"> ' +
            '<div class="panel-heading" data-ng-bind-html="titleText" ng-hide="hideTitle"></div>' +
            '<div class="panel-body"><span class="text-red">{{summaryName}}：</span>{{summaryText||noSummaryText}}</div><div class="arrow" ng-style="arrowposition"><div></div></div>',

        controller: function($scope, $element, $attrs) {
            $scope.hideTitle = $attrs.hideTitle ? $attrs.hideTitle : false;
            $scope.summaryName = $attrs.summaryName ? $attrs.summaryName : '摘要';
        ...
        link: function(scope, element, attrs, ctrl) {
            ...
            scope.mouseenter = function($event) {
                ...
                $timeout(function() {
                    var nextH = $($event.target).next().height();
                    if (elemTp > windowHt - 100) {
                        scope.panelpostion = {
                            top: - nextH,
                            left: $event.offsetX
                        };
        ...
        controller: function($scope, $element, $timeout) {
            ...
            $scope.setCustomOption = function() {
                ...
                if ($scope.isWithoutCompareTime) {
                    ...
                    if ($scope.customOption.startdate && $scope.customOption.enddate) {
