...
controller('resCenRetCtrl', ['$scope', '$filter', '$state', '$stateParams', '$location', '$modal', 'resCtrModalService', 'initComDataService', 'resourceCenterService', 'trsspliceString', 'trsconfirm', 'leftService', '$interval', 'trsHttpService', '$anchorScroll', '$timeout',
    function($scope, $filter, $state, $stateParams, $location, $modal, resCtrModalService, initComDataService, resourceCenterService, trsspliceString, trsconfirm, leftService, $interval, trsHttpService, $anchorScroll, $timeout) {
        ...
        function initStatus() {
            ...
            $scope.params = {
                ...
                keywords: {
                    ...
                    "keyword_and": $stateParams.keyword_and || '', //多值已空格“ ”分隔
                    "keyword_or": $stateParams.keyword_or || '', //多值已空格“ ”分隔
                    "keyword_not": $stateParams.keyword_not || '', //关键字组合,多值已空格“ ”分隔
                    "author": "", //作者, 多值;分隔
                    "area": "", //地域，码值，多值已;分隔
            ...
            //如果是从事件分析详情页的事件主体点出来的直接先检索一次
            $stateParams.fromevent && requestDataPromise();
        }
        ...
        $scope.searchWithTime = function(curTime) {
            if (curTime.name == '自定义') {
                var start = curTime.startdate ? $filter('date')(curTime.startdate, "yyyy-MM-dd").toString() : "";
                var end = curTime.enddate ? $filter('date')(curTime.enddate, "yyyy-MM-dd").toString() : "";
                if (!start && !end) {
                    $scope.params.keywords.time_range = -1;
                } else {
                    $scope.params.keywords.time_range = start + ";" + end;
                }
