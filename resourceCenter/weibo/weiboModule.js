"use strict";
angular.module('resCenWeiboModule', ['resCtrModalModule', 'akoenig.deckgrid'])
    .controller('resCenWeiboMainCtrl', ['$scope', '$q', '$state', '$location', '$modal', 'trsHttpService', 'resCtrModalService', 'initComDataService',
        'resourceCenterService', 'trsspliceString', 'trsconfirm', 'leftService', '$interval', 'fgDelegate', '$timeout',
        'dateFilter',
        function($scope, $q, $state, $location, $modal, trsHttpService, resCtrModalService, initComDataService, resourceCenterService, trsspliceString, trsconfirm, leftService, $interval, fgDelegate, $timeout, dateFilter) {
            ...
            /**
             * [searchWithKeyword description]下拉框回调
             * @return {[type]} [description]
             */
            $scope.searchWithKeyword = function() {
                vm.page.pagenumber = 1; //选择下拉框的时候讲当前页重置
                getListbyItem(listparams, vm.page.pagenumber, vm.page.pagesize);
            };
            var getListbyItem = function(params, pageNum, pageSize, isflow) {
                vm.loadingBusy = true;
                var searchParams = angular.copy(params);
                vm.jumpToPageNum = searchParams.pageNum = vm.page.pagenumber = pageNum ? pageNum : vm.page.pagenumber;
                searchParams.pageSize = vm.page.pagesize = pageSize ? pageSize : vm.page.pagesize;
                var keyword = {};
                ...
                keyword.menuscriptType = vm.docType ? vm.docType.value : ""; //稿件类型
                searchParams.keyword = JSON.stringify(keyword);
                $scope.loadingPromise = resourceCenterService.getListbyItem(searchParams).then(function(data) {
                    if (data.content) {
                        if (vm.page.pagenumber < 2) { //区分加载更多跟普通加载
                            // if (keyword.keywords && !isflow) {
                        ...
                        vm.page.PAGECOUNT = data.summary_info.PAGECOUNT ? data.summary_info.PAGECOUNT : 1;//解决显示加载更多的问题
            ...
            $scope.loadMore = function() {
                getListbyItem(listparams, ++vm.page.pagenumber, vm.page.pagesize, true);
            };
