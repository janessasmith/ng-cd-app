"use strict";
angular.module('resCenDigitalPreviewOtherDetailModule', [])
    .controller('resCenDigitalPreviewOtherDetailCtrl', ['$scope', '$state', '$stateParams', "$q", "$filter", "$timeout", '$interval', 'trsHttpService', 'initComDataService', 'resourceCenterService', 'leftService', 'resCtrModalService', 'trsconfirm', 'trsspliceString', 'dateFilter', 'globleParamsSet', '$window', 'storageListenerService', 'trsPrintService', 'ueditorService',
        function($scope, $state, $stateParams, $q, $filter, $timeout, $interval, trsHttpService, initComDataService, resourceCenterService, leftService, resCtrModalService, trsconfirm, trsspliceString, dateFilter, globleParamsSet, $window, storageListenerService, trsPrintService, ueditorService) {
            initStatus();
            initData();
            /**
            ...
            function initStatus() {
                ...
                $scope.status = {
                    ...
                    hasData: true,
                    paperName: $stateParams.papername,
                };
                $scope.data = {
                    items: [],
                    space: '',
             ...
             */
            function initData() {
                requestBanciDropDown(unescape($stateParams.docpubtime)).then(function(data) {
                    console.log(data);
                    $scope.params.pageUrl = data.content[0].IR_PAGEURL;
                    $scope.data.imgUrl = data.content[0].JPPATH;
                    $scope.data.banmianName = data.content[0].IR_PAGENAME;
                    requestData(false);
                });
            };
            /**
             ...
             * @return {[type]} [description] null
             */
            function requestData(isDrawPoint) {
                $scope.loadingPromise = trsHttpService.httpServer(trsHttpService.getBigDataRootUrl(), $scope.params, 'post').then(function(data) {
                    ...
                    $("<img/>").attr("src", $scope.data.imgUrl).load(function() {
                        ...
                        // $('.left_Img').css({ 'width': realWidth + 5 });
                        ...
                        init_area(realWidth, realHeight);
                    });
                    initOperFlag();
                })
            }
            ...
            function requestBanciDropDown(date) {
                ...
                trsHttpService.httpServer(trsHttpService.getBigDataRootUrl(), banciParams, 'post').then(function(data) {
                    ...
                    var banciList = data.content,
                        index = 0;
                    angular.forEach(banciList, function(value, key) {
                        value.name = value.IR_PAGENAME;
                    });
            ...
            /**
             * [searchBanmian description]选择版面
             */
            $scope.searchBanmian = function(banmian) {
                if ($scope.params.pageUrl == banmian.IR_PAGEURL) return;
                $scope.params.pageUrl = banmian.IR_PAGEURL;
                $scope.data.imgUrl = banmian.JPPATH;
                $scope.data.banmianName = banmian.IR_PAGENAME;
                requestData();
            };
            /**
             * [searchWidthList description]点版面列表搜索
             */
            $scope.searchWidthList = function(banmian) {
                if ($scope.params.pageUrl == banmian.IR_PAGEURL) return;
                $scope.params.pageUrl = banmian.IR_PAGEURL;
                $scope.data.dropDown.selectedBanmian = banmian;
                $scope.data.imgUrl = banmian.JPPATH;
                $scope.data.banmianName = banmian.IR_PAGENAME;
                requestData();
            };
            ...
            $scope.printbtn = function() {
                var params = {
                    'serviceid': $scope.params.serviceid,
                    'modelid': 'epaperDetail',
                    'typeid': 'zyzx',
                    'nodeid': $scope.params.nodeId,
                    'guid': $scope.status.isShowDetail ? $scope.data.currItem.ZB_GUID : trsspliceString.spliceString($scope.data.selectedArray, "ZB_GUID", ","),
                }
            ...
            function CoordMax(coord, w, h, realW, realH) {
                //新宽度
                if (!w)
                    w = 300;
                //新高度
            ...
            function init_area(realWidth, realHeight) {
                var canvans_width = $("#Img_a_canvas").attr("width");
                var canvans_height = $("#Img_a_canvas").attr("height");
                $("#js_tbody").find("a").each(function() {
                    $(this).attr("coords", CoordMax($(this).attr("coords"), canvans_width, canvans_height, realWidth, realHeight));
                });
                $(".forArchives").find("area").each(function() {
                    var $this = $(this);
                    $this.attr("coords", CoordMax($(this).attr("coords"), canvans_width, canvans_height, realWidth, realHeight)); //转变坐标为map区域 坐标 
                    var $x = -70;
                    var $y = -80;
                    $this.mouseover(function(e) {
                        if ($(".mapDiv").length > 0) return;
                        // var name = $(this).attr("alt") == "无标题" ? '' : $(this).attr("alt");
                        var name = $(this).attr("alt");
                        var index_num = $(this).index();
                        var mapClass = name != '无标题' ? 'mapDiv' : 'mapDiv hide';
                        var dom = "<div class='" + mapClass + "'><span class='name'></span></div>";
            ...
            $scope.checkDetail = function(item, index) {
                ...
                trsHttpService.httpServer(trsHttpService.getBigDataRootUrl(), params, 'post').then(function(data) {
                    ...
                    $scope.basicParams.indexName = data.summary_info.indexName;
                    $scope.data.detail.CONTENT = $scope.data.detail.CONTENT.replace(/\r\n/g, "<br/>");
                    $scope.data.detail.CONTENT = $scope.data.detail.CONTENT.replace(/\r/g, "<br/>");
                    $scope.data.detail.CONTENT = $scope.data.detail.CONTENT.replace(/\n/g, "<br/>");
                    $scope.data.detail.CONTENT = dealContent($scope.data.detail.CONTENT);
            ...
            $scope.returnToList = function() {
                if (!$scope.status.isShowDetail) return;
                $scope.status.isShowDetail = false;
                $scope.data.detail = '';
            };
            /**
            ...
            $scope.jumpToDraft = function(isNext) {
                if ((!isNext && $scope.data.currIndex == 0) || (isNext && $scope.data.currIndex == $scope.data.items.length - 1) || !$scope.status.isShowDetail) return;
                ...
                trsHttpService.httpServer(trsHttpService.getBigDataRootUrl(), params, 'post').then(function(data) {
                    ...
                    $scope.data.detail.CONTENT = $scope.data.detail.CONTENT.replace(/\r/g, "<br/>");
                    $scope.data.detail.CONTENT = $scope.data.detail.CONTENT.replace(/\n/g, "<br/>");
                    if ($scope.data.detail.CONTENT) $scope.data.detail.CONTENT = dealContent($scope.data.detail.CONTENT);
                    $scope.status.isShowDetail = true;
            ...
            /**
             * [jumpToBanmian description]上一版/下一版
             */
            $scope.jumpToBanmian = function(isNext) {
                ...
                $scope.params.pageUrl = $scope.data.dropDown.banciList[index].IR_PAGEURL;
                $scope.data.imgUrl = $scope.data.dropDown.banciList[index].JPPATH;
                $scope.data.banmianName = $scope.data.dropDown.banciList[index].IR_PAGENAME;
                requestData();
            };
            /**
            ...
            /**
             * [dealContent description]处理详情内容的格式
             */
            function dealContent(content) {
                return ueditorService.replaceHtmlBlank(content).replace(/(<(?:p|div)[^>]*>(?:<(?:span|b)[^>]*>)*)(?:&nbsp;| |　)*/ig, "$1　　").replace(/(<br[^>]*>)(?:&nbsp;| |　)*/ig, "$1　　").replace(/>　　</g, "\>\<").replace(/style="[^>]*"/g, "");
            }
        }
    ])
