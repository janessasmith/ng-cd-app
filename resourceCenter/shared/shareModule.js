'use strict';
angular.module("resourceCenterSharedModule", ["resCenterGXRouterModule", "resourceCenterSharedMailModule", "resourceCenterSharedRobotDraftModule"]).
controller("resourceCenterSharedCtrl", ['$scope', '$compile', '$q', '$filter', '$window', '$stateParams', '$timeout', '$state', 'initComDataService', 'trsHttpService', 'resourceCenterService', 'leftService', 'resCtrModalService', 'trsconfirm', 'trsspliceString', 'globleParamsSet', 'localStorageService', 'storageListenerService', 'trsPrintService', 'groupUserRightsService', 'editingCenterService',
    function($scope, $compile, $q, $filter, $window, $stateParams, $timeout, $state, initComDataService, trsHttpService, resourceCenterService, leftService, resCtrModalService, trsconfirm, trsspliceString, globleParamsSet, localStorageService, storageListenerService, trsPrintService, groupUserRightsService, editingCenterService) {
        ...
        function requestPrintVersion(item) {
            ...
            $scope.status.loadingPromise = trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), {
        ...
        /** [commonDeleteDrafts 普通删除稿件] */
        $scope.commonDeleteDrafts = function() {
            if (!determineSelected()) return;
            trsconfirm.confirmModel("删除", "是否从共享稿库删除", function() {
                var ids = trsspliceString.spliceString($scope.data.selectedArray, "METADATAID", ",");
                resourceCenterService.commonDeleteRelease(ids, $stateParams.nodeid).then(function(data) {
        ...
        /** [adminDeleteDrafts 管理员删除稿件] */
        $scope.adminDeleteDrafts = function() {
            if (!determineSelected()) return;
            trsconfirm.confirmModel("删除", "是否从共享稿库删除", function() {
                var ids = trsspliceString.spliceString($scope.data.selectedArray, "METADATAID", ",");
                resourceCenterService.adminDeleteRelease(ids, $stateParams.nodeid).then(function(data) {
                    if (data && data.ISSUCCESS == "true") {
                        trsconfirm.alertType("删除成功！", "", "success", false, "");
                        requestData();
                    }
        ...
        function requestData() {
            if ($scope.status.hasphoto) {
                $scope.status.imgFlowPage.CURRPAGE = 1;
                $scope.data.items = [];
                $scope.status.selectedArray = [];
                requestDataImgFlow();
                return;
            }
            var serviceName = "",
        ...
        var allCacheData = [];
        //queryItemsByPanel 第二个参数为回调函数，用于对瀑布流展示做特殊处理
        function queryItemsByPanel(index, imgFlowCallback) {
            if ($scope.data.panels.length < 1) return;
            var params = {
                serviceid: 'mlf_releaseSource',
        ...
        function dynamicRender(index, data, opUsers, images) {
            var html = "";
            var operUsers = [];
            var regx = /"([\s\S]*?)"/gi; //替换标题中的半角双引号
            for (var i = 0; i < data.length; i++) {
                while (regx.test(data[i].TITLE)) {
                    data[i].TITLE = data[i].TITLE.replace(regx, "“" + RegExp.$1 + "”");
                }
        ...
        $scope.fullTextSearch = function(ev) {
            if ((angular.isDefined(ev) && ev.keyCode == 13) || angular.isUndefined(ev)) {
                $state.go("resourcectrl.share.resource1", {
                    ...
                    "sort": true,
                    "estype": $scope.keyType.curValue.value,
                    "estimetype": $scope.time.curValue.value,
                    "esdoctype": $scope.docType.curValue.value
                });
            }
        ...
        /** 切换文字图片列表 */
        $scope.togglehasphoto = function(flag) {
            $scope.status.hasphoto = flag;
            requestData();
            //clearPageData();
        };
        /** [loadMore 加载图片分页] */
        $scope.loadMore = function() {
            $scope.status.imgFlowPage.CURRPAGE = parseInt($scope.status.imgFlowPage.CURRPAGE) + 1;
            requestDataImgFlow();
        ...
        //请求图片流列表
        function requestDataImgFlow() {
            var params = {
                serviceid: "mlf_releasesource",
                methodname: "queryShareDocsOfPic",
                MetaCategoryId: $stateParams.nodeid,
                ShareTime: $scope.time.curValue.value || null,
                IsUnit: $scope.status.isMedia,
                TitleOrder: $stateParams.nodeid == 7 ? 1 : null,
                PAGESIZE: $scope.status.imgFlowPage.PAGESIZE,
                CURRPAGE: $scope.status.imgFlowPage.CURRPAGE
            };
            $scope.status.imgFlowLoad = trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "get").then(function(data) {
                delete $scope.status.imgFlowLoad;
                $scope.data.items = $scope.data.items.concat(data.DATA);
                $scope.status.imgFlowPage.CURRPAGE = data.PAGER.CURRPAGE;
                $scope.status.imgFlowPage.PAGECOUNT = data.PAGER.PAGECOUNT;
            });
        };
    }
]);
