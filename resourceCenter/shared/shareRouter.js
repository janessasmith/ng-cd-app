...
angular.module('resCenterGXRouterModule', []).config(['$stateProvider', function($stateProvider) {
    $stateProvider.state("resourcectrl.share.resource", {
        ...
        url: '/resource1?nodeid&nodename&change&keywords&sort&estype&estimetype&esdoctype',
        views: {
            'resource@resourcectrl.share': {
    ...
    }).state("resourcectrl.share.robotdraft", {
        url: '/robotdraft?nodeid',
        views: {
            'main@resourcectrl': {
                templateUrl: './resourceCenter/shared/robotDraft/robotDraft_tpl.html',
                controller: 'resourceCenterSharedRobotDraftCtrl'
            }
        }
    });
}]);
