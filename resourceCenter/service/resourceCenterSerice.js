...
factory('resourceCenterService', ["trsHttpService", "localStorageService", function(trsHttpService, localStorageService) {
    var rooturl = trsHttpService.getBigDataRootUrl();
    var wcmurl = trsHttpService.getWCMRootUrl();

    function getRealParams() {
        var params = {};
        var orgParams = window.location.href;
        orgParams = orgParams.substring(orgParams.indexOf("?") + 1);
        orgParams = orgParams.split("&");
        for (var i = 0; i < orgParams.length; i++) {
            var objName = orgParams[i].substring(0, orgParams[i].indexOf("="));
            var objValue = orgParams[i].substring(orgParams[i].indexOf("=") + 1);
            params[objName] = objValue;
        }
        return params;
    }

    function getRealRouterName() {
        var curUrl = window.location.href;
        var currentRouter = curUrl.substring(curUrl.indexOf("#/") + 2, curUrl.indexOf("?")).replace(/\//g, ".");
    ...
    return {
        ...
        //共享稿库普通删除
        commonDeleteRelease: function(ids, nodeid) {
            var params = {
        ...
        //共享稿库管理员删除
        adminDeleteRelease: function(ids, nodeid) {
            var params = {
                "methodname": "deleteRelease",
                "serviceid": "mlf_releasesource",
                "MetaDataIds": ids,
        ...
        //资源中心头部标签切换记忆功能
        urlMemory: function($scope) {
            $scope.$on("$destroy", function() {
                var obj = localStorageService.get("resourceCenterUrlMemory");
                var curRouter = getRealRouterName();
                var name = curRouter.split(".")[1];
                ...
                localStorageService.set("resourceCenterUrlMemory", obj);
            });
        }
    };
}]);
