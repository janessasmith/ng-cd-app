...
controller('loginCtrl', ['$scope', '$state', '$modal', '$q', '$validation', '$window', 'localStorageService', 'trsHttpService', 'trsconfirm', 'loginService', '$stateParams', '$timeout', 'versionCtrl',
    function($scope, $state, $modal, $q, $validation, $window, localStorageService, trsHttpService, trsconfirm, loginService, $stateParams, $timeout, versionCtrl) {
        initStatus();
        ...
        function initStatus() {
            ...
            $scope.status = {
                isModalOpened: false,
                isDevelopModal: versionCtrl.isDev || versionCtrl.isDebug,
                isShowCode: '',
                codeImg: '/wcm/verifycode.do',
            ...
            //判断是否显示验证码
            isShowCode().then(function(data) {
                randomCode();
            })
        }
        /**
         * [doLogin description]是否显示验证码
         * @return {[type]} [description]
         */
        function isShowCode() {
            var defer = $q.defer();
            loginService.isShowCode().then(function(data) {
                $scope.status.isShowCode = data;
                defer.resolve(data);
            });
            return defer.promise;
        }
        /**
         * [clickRandomCode description]点击随机验证码
         * @return {[type]} [description]
         */
        $scope.clickRandomCode = function() {
            randomCode();
        };
        /**
         * [randomCode description]随机验证码
         * @return {[type]} [description]
         */
        function randomCode() {
            var random = Math.random();
            $scope.status.codeImg = '/wcm/verifycode.do' + '?' + random;
        };
        /**
        ...
        function doLogin() {
            ...
            $validation.validate($scope.loginForm).success(function() {
                LazyLoad.js(["./lib/js-base64/base64.js"], function() {
                    ...
                    var encryptedCode = "__encoded__" + Base64.encode(angular.copy($scope.data.user.Code));
                    loginService.login(encryptedUserName, encryptedPassword, encryptedCode).then(function(data) {
