...
angular.module('editctrSupportCreationModule', [
        ...
        return {
            ...
            link: function($scope, iElm, iAttrs, controller) {
                ...
                function querySubject(isGetMore) {
                    if (promiseB) {
                        $timeout.cancel(promiseB);
                        promiseB = null;
                    }
                    promiseB = $timeout(function() {
                        var params = {
                            typeid: "zyzx",
                            serviceid: "write",
                        ...
                        $scope.loadingSubject = trsHttpService.httpServer(trsHttpService.getBigDataRootUrl(), params, "post").then(function(data) {
                            if (data.result === "fail" || data === "" || data.result === "warn") return;
                            if (!!isGetMore)
                ...
                /**
                 * [getBackmeanu description]获得稿件背景
                 * @return {[type]} [description]
                 */
                $scope.getBackmeanu = function() {
                    queryBackmeanu(false);
                };
                /**
                ...
                function queryBackmeanu(isGetMore) {
                    if (promiseC) {
                        $timeout.cancel(promiseC);
                        promiseC = null;
                    }
                    promiseC = $timeout(function() {
                        var params = {
                            typeid: "zyzx",
                            serviceid: "write",
                        ...
                        $scope.loadingSubject = trsHttpService.httpServer(trsHttpService.getBigDataRootUrl(), params, "post").then(function(data) {
                            if (data === "" || data.result === "fail" || data.result === "warn") return;
                            if (!!isGetMore) {
    ...
    .directive('plancenterMoreinfos', ['$timeout', 'trsHttpService', 'initVersionService', 'alterModule', 'plancenterService', function($timeout, trsHttpService, initVersionService, alterModule, plancenterService) {
        return {
            // name: '',
            // priority: 1,
            // terminal: true,
            ...
            link: function($scope, iElm, iAttrs, controller) {
                ...
                /**
                 * [querySimilar description:查询相似报题]
                 * @param  {Boolean} isGetMore [description：判断是否加载跟多]
                 */
                function querySimilar(isGetMore) {
                    var params = {
                        KeyWords: $scope.content
                    };
                    $scope.loadingSubject = plancenterService.invoke('querySelectedTopicsByKeyWords', params).then(function(data) {
                        if (!isGetMore) {
                            $scope.status.similaritems = data;
                        } else {
                            $scope.status.similaritems = $scope.status.similaritems.concat(data.DATA);
                        }
                    });
                }
            }
