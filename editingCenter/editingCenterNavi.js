        ...
        .controller('EditingCenterNaviCtrl', ['$scope', '$state', '$filter', '$location', '$q', 'trsHttpService',
            function($scope, $state, $filter, $location, $q, trsHttpService) {
                ...
                $scope.setCurrProduct = function(product) {
                    ...
                    //     queryWebsites();
                    // }
                    queryWebsites().then(function(data){
                        $scope.setCurrSite($scope.data[$scope.status.currProduct.router].sites[0]);
                    });
                        
                };
                //设置当前网站
                $scope.setCurrSite = function(site) {
                    if($scope.status.currNavi == site) return;
                    $scope.status.currNavi = site;
                    $state.go('editctr.' + $scope.status.currProduct.router, { siteid: site.SITEID, paperid: site.SITEID, channelid: site.CHANNELID });
                };
