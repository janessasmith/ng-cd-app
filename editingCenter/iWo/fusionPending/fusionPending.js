    ...
    .controller('fusionPendingCtrl', ["$stateParams", "$filter", '$q', '$state', '$window', '$cacheFactory', '$scope', '$location', 'localStorageService', '$anchorScroll', 'trsHttpService', "trsconfirm", "$document", "$popover", "initVersionService", "$modal", "trsspliceString", "$timeout", "trsResponseHandle", 'initSingleSelecet', '$interval', 'editcenterRightsService', 'iWoService', 'editingCenterService', 'editNewspaperService', "trsPrintService", "storageListenerService", "pagePreview", "editIsLock",
        function($stateParams, $filter, $q, $state, $window, $cacheFactory, $scope, $location, localStorageService, $anchorScroll, trsHttpService, trsconfirm, $document, $popover, initVersionService, $modal, trsspliceString, $timeout, trsResponseHandle, initSingleSelecet, $interval, editcenterRightsService, iWoService, editingCenterService, editNewspaperService, trsPrintService, storageListenerService, pagePreview, editIsLock) {
            ...
            function addItemsToPanel(isGetMore, index, data) {
                ...
                //非报纸类简单数据类型
                if ($scope.status.currPanel.MEDIATYPE === $scope.data.mediaType.website || $scope.status.currPanel.MEDIATYPE === $scope.data.mediaType.app) {
                    //判断是否记载更多
                    if (isGetMore) {
            ...
            $scope.queryByDropdown = function(key, value) {
                ...
                $scope.status.cachedPage = {};
                $scope.params.CURRPAGE = $scope.status.copyCurrPage = $scope.status.cachedPage[$scope.status.currPanel.SITEID.toString()] = 1;
                //$scope.params.CurrPage = $scope.status.copyCurrPage = $scope.page.CURRPAGE = $scope.status.cachedPage[$scope.status.currPanel.SITEID.toString()] =1;              
                if (key == 'OprTime') {
                    if (value.length < 10) {
                ...
                $scope.status.loadingPromise = trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, 'get').then(function(data) {
                    ...
                    if (flag) {
                        for (var i in $scope.data.panels) {
                            var item = $scope.data.panels[i];
                            if (item.DOCCOUNT > 0) {
                                $scope.status.openedPanel[i] = true;
                                $scope.status.currPanel = item;
                                queryItemsBySiteId(false);
                                return false;
                            }
                        }
            ...
            $scope.printBtn = function() {
                if ($scope.status.currPanel.MEDIATYPE === $scope.data.mediaType.newspaper) {
                    printNewspaper();
                } else if ($scope.status.currPanel.MEDIATYPE === $scope.data.mediaType.website) {
                    printWebsite();
                }
            };
            ...
            function printWebsite() {
                ...
                angular.forEach($scope.data.selectedArray, function(value, key) {
                    requestPrintVersion(value).then(function(data) {
                        requestWebsitePrintData(value, data);
                    });
                });
            };
            /**
             ...
             */
            function requestWebsitePrintData(item, version) {
                var params = {
                    "serviceid": "mlf_website",
                    "methodname": $scope.status.websiteDetailMethodname[item.DOCTYPEID],
                    "MetaDataId": item.METADATAID
                };
                $scope.loadingPromise = trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "post").then(function(data) {
                    ...
                    $scope.status.websitePrintResult.push(result);
                    if ($scope.status.websitePrintResult.length == $scope.data.selectedArray.length) {
                        trsPrintService.trsWebPrintDocument($scope.status.websitePrintResult);
                        $scope.status.websitePrintResult = [];
                    }
                });
            }
            /**
