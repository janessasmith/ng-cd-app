...
*/
"use strict";
angular.module("myManuscriptService", ["iWoDraftModule", "iWoSbumitModule", "copyBuildDraftModule", "getDraftModule", "iWoRejectionModule", "draftToDepartmentModule"]).
factory("myManuscriptService", ["$modal", "$q", function($modal, $q) {
    return {
        draft: function(title, array, success, fail, methodname, returnArray, recentOperator) {
        ...
        /**
         * [draftToDepartment description]稿件提交到部门稿库&部门稿库传递
         * @param  {[arr]} items  [description]选中的集合
         * @return {[type]}       [description]
         */
        draftToDepartment: function(title, items) {
            var defer = $q.defer();
            var modalInstance = $modal.open({
                templateUrl: "./editingCenter/iWo/myManuscript/service/department/draftToDepartment_tpl.html",
                windowClass: "newspaper-shangban-windows",
                backdrop: false,
                controller: "draftToDepartmentCtrl",
                resolve: {
                    title: function() {
                        return title||"稿件提交"; //增加title的默认值
                    },
                    items: function() {
                        return items;
                    }
