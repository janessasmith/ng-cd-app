...
controller('iWoSubmitCtrl', ['$scope', '$state', '$rootScope', '$timeout', '$q', '$filter', '$modalInstance', 'trsHttpService', 'trsResponseHandle', 'SweetAlert', 'trsspliceString', 'trsconfirm', 'selectedArray', 'iWoSbumitService', "methodname", "editingCenterService", "trsColumnTreeLocationService",
    function($scope, $state, $rootScope, $timeout, $q, $filter, $modalInstance, trsHttpService, trsResponseHandle, SweetAlert, trsspliceString, trsconfirm, selectedArray, iWoSbumitService, methodname, editingCenterService, trsColumnTreeLocationService) {
        initStatus();
        initData();
        //初始化状态函数
        function initStatus() {
        ...
        $scope.confirm = function() {
            ...
            params.serviceid = $state.params.departmentid || $state.params.isdepart ? "nb_departmentRelease" : "mlf_myrelease"; //区分是部门稿库还是个人稿库
            params.DepartmentId = $state.params.departmentid ? $state.params.departmentid : null;
            //根据pathArray区分提交渠道
            angular.forEach($scope.path.pathArray, function(value, key) {
                if(value.params=='app') value.params="MlfApp";//处理app渠道存在问题
            ...
            $scope.loadingPromise = trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "get").then(function(data) {
                trsconfirm.alertType("上栏成功", "", "success", false, function() {
