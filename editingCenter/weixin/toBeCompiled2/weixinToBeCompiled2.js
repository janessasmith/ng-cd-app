"use strict";
angular.module('weixinTobeCompiledTwoModule', [
    'weixinTobeCompiledTwoRouter',
    ...
    'weiXinAudioModule',
    'weiXinVideoModule',
    ]).controller('WeiXinTobeCompiledTwoCtrl', ['$scope', '$filter', '$q', '$modal', '$stateParams', '$state', '$window', 'editIsLock', 'localStorageService', 'trsHttpService', 'SweetAlert', 'initSingleSelecet', 'editingCenterService', 'trsconfirm', 'trsspliceString', 'initVersionService', 'editcenterRightsService', 'signedService', 'storageListenerService', 'globleParamsSet', 'trsPrintService', 'WeiXininitService', function($scope, $filter, $q, $modal, $stateParams, $state, $window, editIsLock, localStorageService, trsHttpService, SweetAlert, initSingleSelecet, editingCenterService, trsconfirm, trsspliceString, initVersionService, editcenterRightsService, signedService, storageListenerService, globleParamsSet, trsPrintService, WeiXininitService) {
    initStatus();
    initData();
    ...
    function initStatus() {
        ...
        $scope.status = {
            ...
            // 要跳转的页面
            copyCurrPage: 1,
            wxStatus: 'toBeCompiled2', //微信状态：待编
        };
        $scope.data = {
            items: [],
            selectedArray: [],
