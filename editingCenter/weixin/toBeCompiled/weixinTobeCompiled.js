"use strict";
angular.module('weixinTobeCompiledModule', [
    'weixinTobeCompiledRouter',
    'wechatGraphicModule',
    'wechatPictureModule',
    'wechatTextModule',
    'weiXinAudioModule',
    'weiXinVideoModule',
    ]).controller('WeiXinTobeCompiledCtrl', ['$scope', "$filter", "$q", "$modal", "$state", "$stateParams", "trsHttpService", "$timeout", "$window", "trsconfirm", "storageListenerService", "initSingleSelecet", "editingCenterService", "trsspliceString", "trsPrintService", "WeiXininitService", "editcenterRightsService", "globleParamsSet", "editIsLock","editingCenterWechatService","jsonArrayToStringService",
        function($scope, $filter, $q, $modal, $state, $stateParams, trsHttpService, $timeout, $window, trsconfirm, storageListenerService, initSingleSelecet, editingCenterService, trsspliceString, trsPrintService, WeiXininitService, editcenterRightsService, globleParamsSet, editIsLock, editingCenterWechatService,jsonArrayToStringService) {
    initStatus();
    initData();
    ...
    function initStatus() {
        $scope.data = {
            ...
            wxchannelid: $state.params.channelid,//微信账号ID
            wxId: '', //微信原始ID
        };
        $scope.page = {
            "CURRPAGE": 1,
        ...
        $scope.status = {
            ...
            params: {
                ...
                "methodname": "queryWeChatDoc",
                "WXChannelId": $stateParams.channelid,
                "timeType": "",
                "DocType": ""
            },
            copyCurrPage: 1,
            ...
            onlyMine: false,
            isESSearch: false,
            wxStatus:'tobecompiled', //微信状态：待编
            wxgraphicDocType: '1', //图文稿件类型为1
        };
    }
    ...
    /**
     * [getWechatId description] 获取微信原始id
     * @return {[type]} [description]
     */
    function getWechatId() {
        editingCenterWechatService.getWechatId($state.params.channelid).then(function(data) {
            $scope.data.wxId = data.result[0].wxid;
        });
    }
    ...
    $scope.trial = function() {
        ...
        if($scope.data.selectedArray.length === 1){
            draftTrial(JSON.stringify([$scope.data.selectedArray[0].METADATAID]));
        } else {
            var isGraphic = true;
            angular.forEach($scope.data.selectedArray, function(data, index, array) {
                if(data.WECHATDOCTYPE != $scope.status.wxgraphicDocType) {
                    isGraphic = false;
                }
            });
            if(isGraphic) {
                //因获取的列表数据中没有图文稿的图片，所以传递metaDataId, 在弹框中重新获取稿件信息
                var metaDataIds = trsspliceString.spliceString($scope.data.selectedArray, "METADATAID", ",");
                editingCenterWechatService.multiDraftPend(metaDataIds, $scope.data.selectedArray[0].WXCHANNELID).then(function(data) {
                    draftTrial(data);
                });
            } else {
                trsconfirm.alertType("送审失败", "所选稿件不全属于图文稿件类型", "error", false);
            }
        }
        /*
        var metaDataIds = trsspliceString.spliceString($scope.data.selectedArray, "METADATAID", ",");
    ...
    /**
     * [DraftTrial description] 组稿送审
     * @return {[type]} [description]
     */
    function draftTrial(ids) {
        var params = {
            serviceid: "wcm61_wxmessage",
            methodname: "sendMessage",
            MsgType: $scope.data.MsgType[$scope.data.selectedArray[0].WECHATDOCTYPE],
            nodeId: $scope.data.wxchannelid,
            GroupId: -1000,
            GroupName: "全部用户",
            Country: "",
            wxId: $scope.data.wxId,
            wxdocids: ids,
        };
