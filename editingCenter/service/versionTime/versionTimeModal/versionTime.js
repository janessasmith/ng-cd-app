...
controller('editiWoObjTimeCtrl', ['$scope', '$location', '$filter', '$sce', '$q', '$modalInstance', 'trsHttpService', "trsconfirm", "incomingData", "initVersionService", function($scope, $location, $filter, $sce, $q, $modalInstance, trsHttpService, trsconfirm, incomingData, initVersionService) {
    initStatus();
    initData();
    ...
    function getVersionTime() {
        ...
        trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "get").then(function(data) {
            ...
            if ($scope.status.trench !== 'iWo') {//根据渠道选择不同的处理方式，除Iwo渠道按操作日志方式搞
                $scope.data.copyVersionTime = $filter('groupBy')(data, 'MEDIATYPENAME');
                for (var i in $scope.data.copyVersionTime) {
                    $scope.data.versionTime[i] = [];
                    $scope.data.versionTime[i] = $scope.data.versionTime[i].concat(initVersionService.getDayContent($scope.data.copyVersionTime[i]));
                }
            } else {
