...
controller('shareCtrl', ["$scope", "$timeout", "$modalInstance", "trsHttpService", "SweetAlert", "trsResponseHandle", "trsspliceString", "trsconfirm",
    function($scope, $timeout, $modalInstance, trsHttpService, SweetAlert, trsResponseHandle, trsspliceString, trsconfirm) {
        init();
        //弹窗初始化
        ...
        $scope.selectMClass = function(item) {
            if (!angular.isDefined($scope.mClassSelected[item.ID])) {
                $scope.mClassSelected[item.ID] = item;
                if ($scope.selectedRange === null) {
                    $scope.selectedRange = $scope.unifiedFeeds;
                }
            } else {
                delete $scope.mClassSelected[item.ID];
            }
        };
        //选择范围
        ...
        //删除常用模板
        $scope.deleteTemplate = function(item, ev) {
            ev.stopPropagation();
            trsconfirm.confirmModel("您确定要删除?", "您确定要删除?", function() {
                ...
                trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "post")
                    .then(function(data) {
                        if (item.SHARETEMPLATEID == $scope.selectedTemplate.SHARETEMPLATEID) $scope.selectedTemplate = {};
                        getShareTemplate();
                    });
            });
        };
        ...
        function getMClassification() {
            var params = {
                ...
                methodname: "queryCategorysWithRightOfTouGao",
            };
            trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "get")
                .then(function(data) {
                    for (var i = 0; i < data.length; i++) {
                        if (!data[i].CHILDLIST) data[i].CHILDLIST = [];
                    }
        ...
        function selfPath(data) {
            for (var i = 0; i < data.CHILDREN.length; i++) {
                if (data.CHILDREN[i].ISPATH === "true") {
                    if (data.CHILDREN[i].HASCHILDREN === "false" || data.CHILDREN[i].CHILDREN.length === 0) {
                        $scope.selectedNode = data.CHILDREN[i];
                        getGroupData($scope.selectedNode);
                    } else {
                        $scope.expandedTest.push(data.CHILDREN[i]);
                        selfPath(data.CHILDREN[i]);
                    }
                }
            }
        }
