        ...
        function($scope, $compile, $filter, $q, $window, $state, $modal, $timeout, $location, $validation, localStorageService, jsonArrayToStringService, trsResponseHandle, initeditctrBtnsService, initVersionService, initAddMetaDataService, $stateParams, $anchorScroll, trsHttpService, SweetAlert, initSingleSelecet, trsconfirm, trsspliceString, editingCenterService, $sce, initWebsiteDataService, websiteService, filterEditctrBtnsService, initWebsiteNewsService, trsSelectDocumentService, Upload, websiteBtnService, iWoService, storageListenerService, ueditorService, editIsLock, uploadAudioVideoService, editPictureService) {
            ...
            function manageEditData(data) {
                trsHttpService.httpServer("/wcm/mlfcenter.do", $scope.params, "get").then(function(data) {
                    ...
                    //初始化处理相关图片,仅编辑时生效
                    /*$scope.list.READINGPICS = angular.isUndefined($scope.list.READINGPICS) ? "" : $scope.list.READINGPICS;
                    $scope.list.READINGPICS = doWithPicsOrAttach($scope.list.READINGPICS);*/
                    //初始化处理概览图片,仅编辑时生效
            ...
            $scope.chooseFile = function() {
                ...
                $timeout(function() {
                    $('.uploadPromise').find('.cg-busy-default-text').text('上传中...');
                });
            ...
            function manageDataBeforeSave() {
                ...
                list.DOC_PICTURELIST = angular.copy(jsonArrayToStringService.clearEmptyObjects(list.DOC_PICTURELIST, "APPFILE"));
                /*}
                if (list.OUTLINEPICS.length > 0) {*/
            ...
            $scope.multiImgsDownload = function() {
                var params = {
                    ...
                    "ObjectIds": trsspliceString.spliceString($scope.data.uploaderImgSelected, 'APPENDIXID', ',')
                };
            ...
            $scope.addUploaderImg = function(array, type) {
                addUploadImg(array, type);
            };
            //展示导图位置框
            function addUploadImg(array, type) {
                if (array.length > 9) {
            ...
            //图片列表的图片：设为导图
            $scope.setMap = function(image) {
                var params = {
                    "serviceid": "mlf_websiteoper",
                    "methodname": "setOutLinePics",
                    "FileName": image.APPFILE,
                    "MetaDataId": $stateParams.metadataid ? $stateParams.metadataid : 0
                };
                trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, 'post').then(function(data) {
                    var outLinePic = data;
                    showOutLinePic(outLinePic);
                });
            };
            //展示图片列表的图到导读位置
            function showOutLinePic(image) {
                addUploadImg($scope.list.OUTLINEPICS, '概览图片');
                var idx = $scope.list.OUTLINEPICS.length - 1;
                $scope.list.OUTLINEPICS[idx].APPDESC = image.APPDESC;
                $scope.list.OUTLINEPICS[idx].APPFILE = image.APPFILE;
                $scope.list.OUTLINEPICS[idx].PERPICURL = image.PERPICURL;
            }
            /************************************图片相关操作结束****************************************************/
