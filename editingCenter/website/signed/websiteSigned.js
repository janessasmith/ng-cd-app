...
function signed($scope, $filter, $q, $modal, $stateParams, $state, $window, editIsLock, localStorageService, trsHttpService, SweetAlert, initSingleSelecet, editingCenterService, trsconfirm, trsspliceString, websiteService, initVersionService, editcenterRightsService, signedService, storageListenerService, globleParamsSet, trsPrintService) {
    ...
    $scope.pushBar = function(item) {
        $scope.status.isESSearch = false;
        websiteService.pushBar("推首页（北方网）",function(result) {
            var params = angular.copy(result);
            params.chnldocIds = trsspliceString.spliceString(item ? [item] : $scope.data.selectedArray, "CHNLDOCID", ",");
            promptRequest(params, "推首页成功");
        }, $scope.data.selectedArray);
    };
    /**
     * [pushBarToMlf description]推首页到媒立方
     * @return {[type]} [description] null
     */
    $scope.pushBarToMlf = function(item) {
        $scope.status.isESSearch = false;
        websiteService.pushBarToMlf("推首页（媒立方）",function(result) {
            var params = angular.copy(result);
            params.chnldocIds = trsspliceString.spliceString(item ? [item] : $scope.data.selectedArray, "CHNLDOCID", ",");
            promptRequest(params, "推首页成功");
        }, $scope.data.selectedArray);
    ...
    function copyDraft(array) {
        ...
        for (var i = 0; i < array.length; i++) {
            if (array[i].CHNLID != $stateParams.channelid) {
                isCurrChnl = false;
                break;
            }
        }
        if (!isCurrChnl) {
            trsconfirm.alertType("无法复制", "不能对子栏目的稿件进行复制操作", "error", false, function() {
                return;
            });
        } else {
        ...
        var esParams = {
            ...
            searchParams: {
                ...
                searchFields: [{
                    ...
                    searchField: "isOnlyCurChannel",
                    keywords: $scope.status.params.IsCurrentChnl
                }, {
