 ...
 * Time:2016-02-25
 */
angular.module('websitePreviewModule', []).controller('websitePreviewCtrl', ["$scope", "$sce", "$q", "$modal", "$timeout", "$state", "$location", "$window", "trsHttpService", "$stateParams", 'editcenterRightsService', 'trsconfirm', 'initVersionService', 'editingCenterService', 'websiteService', 'trsPrintService', 'editIsLock', 'trsPicturePreviewService','uploadAudioVideoService',function($scope, $sce, $q, $modal, $timeout, $state, $location, $window, trsHttpService, $stateParams, editcenterRightsService, trsconfirm, initVersionService, editingCenterService, websiteService, trsPrintService, editIsLock, trsPicturePreviewService,uploadAudioVideoService) {
    initStatus();
    initData();
    /**
    ...
    function initData() {
        LazyLoad.css('./components/util/ueditor/service/css/ueditorBuiltInStyles.css?v=2.0', function(arg) {
            $scope.loadingPromise = trsHttpService.httpServer("/wcm/mlfcenter.do", $scope.data.params, "get").then(function(data) {
                ...
                $scope.item.AUDIOVIDEO = $scope.item.AUDIOVIDEO === "" ? [] : $scope.item.AUDIOVIDEO.split(","); //将音视频文件转换为数组格式
                angular.forEach($scope.item.AUDIOVIDEO, function(_data, index, array) {
