...
angular.module('editingCenterSignedModule', [
    ...
    .controller("editingCenterSignedappCtrl", ["$stateParams", "$q", "$filter", "trsHttpService", "$scope", "$modal", "$window", "$state", "trsconfirm", "trsspliceString", "$timeout", "initSingleSelecet", 'editingCenterService', 'storageListenerService', 'trsPrintService', 'globleParamsSet', 'editingCenterAppService', 'editcenterRightsService', "editIsLock",
        function($stateParams, $q, $filter, trsHttpService, $scope, $modal, $window, $state, trsconfirm, trsspliceString, $timeout, initSingleSelecet, editingCenterService, storageListenerService, trsPrintService, globleParamsSet, editingCenterAppService, editcenterRightsService, editIsLock) {
            initStatus();
            initData();
            ...
            $scope.move = function() {
                $scope.status.isESSearch = false;
                var promise=editingCenterAppService.moveDraft('移动稿件', $stateParams.siteid, $stateParams.channelid, 5, function(data) {
                    var chnlDocIds = trsspliceString.spliceString($scope.data.selectedArray, "CHNLDOCID", ",");
                    var metaDataId = trsspliceString.spliceString($scope.data.selectedArray, "METADATAID", ",");
                    var params = {
                        "serviceid": "mlf_appoper",
            ...
            //普通拖拽
            function commonDrag(item, index) {
                if (!$scope.status.btnRights['app.yiqianfa.gaojianpaixu']) {
                    trsconfirm.alertType("登陆用户没有权限对稿件进行排序", "", "error", false);
                    return;
                }
                var trueIndex;
                var params = {
            ...
            $scope.stick = function() {
                ...
                trsconfirm.confirmModel(topFlag === 2 ? "置顶？" : "取消置顶？", topFlag === 2 ? "确认要置顶该稿件？" : "确定要取消置顶该稿件？", function() {
                    ...
                    trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "get").then(function() {
                        var info = topFlag === 2 ? "置顶" : "取消置顶";
                        trsconfirm.alertType(info + '成功', '文章已成功' + info, "success", false);
                        requestData();
                    }, function() {
                        requestData();
                    });
            ...
            /**
             * [messagePush description]消息推送
             * @return {[type]} [description]
             */
            $scope.messagePush = function() {
                trsconfirm.confirmModel("稿件推送", "确认稿件推送", function() {
                    var params = {
                        serviceid: "mlf_appoper",
                        methodname: "push",
                        ChannelId: $state.params.channelid,
                        MetaDataId: trsspliceString.spliceString($scope.data.selectedArray, 'METADATAID', ','),
                    };
                    trsHttpService.httpServer(trsHttpService.getWCMRootUrl(), params, "get").then(function(data) {
                        trsconfirm.alertType('推送成功', '稿件已成功推送', "success", false);
                        requestData();
                    });
                });
            };
    ...
    /**
     * created by zss in 2016/12/23
     * 资源中心app已签发官员列表
     */
    .controller('editingCenterSignedofficerCtrl', ['$scope', '$state', '$stateParams', 'trsHttpService', 'globleParamsSet', 'trsspliceString', 'trsconfirm', 'storageListenerService', 'initSingleSelecet', 'editingCenterService', 'editcenterRightsService',
        function($scope, $state, $stateParams, trsHttpService, globleParamsSet, trsspliceString, trsconfirm, storageListenerService, initSingleSelecet, editingCenterService, editcenterRightsService) {
             ...
             */
            $scope.retraction = function() {
                trsconfirm.inputModel('取消签发', '请输入取消签发原因', function(content) {
                    var params = {
                        "serviceid": "mlf_appoper",
                        "methodname": "withdraw",
